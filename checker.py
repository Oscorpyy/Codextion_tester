        ...
        with open(tests_file, "r") as f:
            for line in f:
                ...
                if len(parts) >= 3:
                    ...
                    if args_str:
                        if "invalid_args" not in parts[0] and "makefile" not in parts[0]:
...
def compile_project():
    print("🔨 Compiling...")
    res = run(["make", "--no-print-directory", "-s"], stdout=subprocess.DEVNULL)
    if res:
        ...
        for line in lines:
            ...
            if len(parts) >= 3:
                try:
                    ...
                    if ts < last_ts:
                        if chrono_ok:
                            print(f"❌ Timestamp moving backwards from {last_ts} à {ts} !")
                            chrono_ok = False
                        ok = False
                    last_ts = ts
                    ...
                    if cid in coder_last:
                        ...
                        needed = 0
                        if "is compiling" in lstate: needed = t_compile
                        elif "is debugging" in lstate: needed = t_debug
                        elif "is refactoring" in lstate: needed = t_refactor
                        
                        if ts - lts < needed and "burned out" not in state:
                            print(f"❌ Invalid duration : Coder {cid} starts '{state}' at {ts}ms, but previous step '{lstate}' ({lts}ms) required {needed}ms (diff = {ts-lts})")
                            ok = False
                            
                    if "has taken a dongle" in state:
                        if ts < last_release_ts + t_cd:
                            print(f"❌ Cooldown not respected : Coder {cid} takes a dongle at {ts}ms, while the last compilation finished at {last_release_ts}ms and the cooldown ({t_cd}ms) lasts until {last_release_ts + t_cd}ms !")
                            ok = False
                    ...
                    pass
        if ok:
            print("✅ Timings (compile, debug, refactor) and Cooldowns are perfectly respected!")
        return ok
    except Exception as e:
...
def valgrind_tests():
    print(f"\n🧠 Valgrind on {len(TESTS)} tests (std output ignored, logs in {VAL_LOG})")
...
def helgrind_tests():
    print(f"\n🧵 Helgrind on {len(TESTS)} tests (std output ignored, logs in {HEL_LOG})")
